...
from sliding_chunks import mask_invalid_locations


class LongformerGPT2LMHeadModel(GPT2LMHeadModel):
    def __init__(self, config):
        super(LongformerGPT2LMHeadModel, self).__init__(config)
        if config.attention_mode == 'n2':
            pass  # do nothing, use BertSelfAttention instead
        else:
            for i, layer in enumerate(self.transformer.h):
                layer.attn = LongformerSelfAttention(config, layer_id=i)


class LongformerGPT2Config(GPT2Config):
...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.query = nn.Linear(config.n_embd, self.embed_dim)
        self.key = nn.Linear(config.n_embd, self.embed_dim)
        self.value = nn.Linear(config.n_embd, self.embed_dim)

        self.query_global = nn.Linear(config.n_embd, self.embed_dim)
        self.key_global = nn.Linear(config.n_embd, self.embed_dim)
        self.value_global = nn.Linear(config.n_embd, self.embed_dim)
